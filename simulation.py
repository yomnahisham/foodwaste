...
def compare_strategies(num_stores: int = 10, num_customers: int = 100, n: Optional[int] = None,
    ...
    if use_parallel and cpu_count() > 1:
        ...
        strategy_configs = [
            ...
            (Hybrid_Enhanced_Strategy(), "Hybrid_Enh", stores, customers, n_value, duration, seed, output_dir, verbose, num_days),
        ]
        
        with Pool(min(5, cpu_count())) as pool:
            results = pool.map(run_strategy_wrapper, strategy_configs)
        
        greedy_results, near_optimal_results, rwes_t_results, yomna_results, hybrid_enhanced_results = results
    else:
    ...
    if verbose:
        print("\nRunning Hybrid Enhanced Strategy (New)...")
    hybrid_enhanced_strategy = Hybrid_Enhanced_Strategy()
    hybrid_enhanced_results = run_single_strategy_simulation(
            hybrid_enhanced_strategy, "Hybrid_Enh", stores, customers, n_value, duration, seed, output_dir, verbose, num_days
        )
    ...
    comparison = {
        ...
        'hybrid_enhanced': calc_metrics(hybrid_enhanced_results)
    }
    ...
    for name, res in [('greedy', greedy_results), ('near_optimal', near_optimal_results),
                      ('rwes_t', rwes_t_results), ('yomna', yomna_results), ('hybrid_enhanced', hybrid_enhanced_results)]:
        if res is not None:
    ...
    if verbose:
        ...
        for i, metric in enumerate(comparison['metric']):
            vals = {
                ...
                'Hybrid': comparison['hybrid_enhanced'][i]
            }
            if has_anan:
        ...
        file_num += 1
        print(f"  {file_num}. Hybrid En. Strategy KPIs: {os.path.join(output_dir, 'hybrid_enhanced_strategy_kpis.csv')}")
    ...
    return {
        ...
        'hybrid_enhanced_results': hybrid_enhanced_results,
