...
@register_strategy("Hybrid_Enhanced_Strategy")
class Hybrid_Enhanced_Strategy(RankingStrategy):
    """
    Hybrid strategy that combines:
    - Near-Optimal's adaptive weights and supply-demand matching
    - Anan's superior fairness via exposure balancing
    - Yomna's conservative safety gates
    - Enhanced personalization to reduce bounce rate
    """
    
    def __init__(self):
        super().__init__()
    
    def select_stores(self, customer, n, all_stores, t=0):
        """
        Enhanced hybrid selection combining best features from all strategies.
        """
        if not all_stores:
            return []
        ...
        max_inventory = max((s.est_inventory for s in all_stores), default=1.0)
        total_stores = len(all_stores)
        max_exposure = max((s.exposure_count for s in all_stores), default=1)
        
        # Customer personalization (from Near-Optimal + Enhanced)
        order_counts = {}
        if hasattr(customer, 'history') and 'orders' in customer.history:
            for rid in customer.history['orders']:
                order_counts[rid] = order_counts.get(rid, 0) + 1
        ...
        # Market conditions (from Near-Optimal)
        total_reservations = sum(s.reservation_count for s in all_stores)
        total_capacity = sum(s.est_inventory for s in all_stores)
        market_utilization = total_reservations / max(1.0, total_capacity)
        
        # Adaptive weights (from Near-Optimal) with INCREASED fairness weight
        if market_utilization > 0.8:
            supply_weight = 0.35  # DECREASED from 0.38
            revenue_weight = 0.25  # INCREASED from 0.20
            fairness_weight = 0.15  # DECREASED from 0.18
            personalization_weight = 0.12
            risk_weight = 0.07
            safety_weight = 0.05
        elif market_utilization < 0.3:
            supply_weight = 0.26
            revenue_weight = 0.35  # INCREASED from 0.28
            fairness_weight = 0.12  # DECREASED from 0.20
            personalization_weight = 0.14
            risk_weight = 0.05
            safety_weight = 0.07
        else:
            supply_weight = 0.34
            revenue_weight = 0.30  # INCREASED from 0.22
            fairness_weight = 0.12  # DECREASED from 0.20
            personalization_weight = 0.12
            risk_weight = 0.06
            safety_weight = 0.06
        ...
        for store in available:
            # 1. SUPPLY-DEMAND MATCHING (from Near-Optimal)
            safe_capacity = max(0, store.est_inventory - store.reservation_count)
            capacity_utilization = store.reservation_count / max(1.0, store.est_inventory)
            ...
            supply_demand_score = (
                0.5 * safe_capacity_ratio +
                0.5 * utilization_score
            )
            
            # 2. RISK-ADJUSTED REVENUE (from Near-Optimal)
            base_revenue = (store.price / max_price) * (store.rating / 5.0)
            ...
            # 3. FAIRNESS SCORE (from Anan - ENHANCED)
            # Strong exposure balancing to ensure all stores get shown
            exposure_penalty = store.exposure_count / max(1.0, max_exposure)
            fairness_score = 1.0 - (exposure_penalty ** 0.5)  # Square root to soften penalty
            
            # 4. PERSONALIZATION SCORE (ENHANCED)
            personal_score = 0.0
            if store.restaurant_id in fav_ids:
                personal_score += 0.6  # Strong boost for favorites
            if store.category in preferred_cats:
                personal_score += 0.4  # Boost for preferred categories
            personal_score = min(1.0, personal_score)
            
            # 5. RISK/SAFETY SCORE (from Yomna)
            # Stricter safety requirements
            risk_penalty = 0.0
            if store.accuracy_score < 0.65:
                risk_penalty += 0.5  # High penalty for low accuracy
            if capacity_utilization > 0.85:
                risk_penalty += 0.3  # Penalty for high utilization
            if capacity_utilization > 0.75 and store.accuracy_score < 0.75:
                risk_penalty += 0.2  # Penalty for risky combo
            
            safety_score = max(0.0, 1.0 - risk_penalty)
            ...
            final_score = (
                ...
                risk_weight * (1.0 - cancellation_prob) +
        ...
        # Select top n with fairness boost
        # Give occasional boost to under-exposed stores
        selected = []
        used_ids = set()
        
        # Take top 90% by score (Increased from 70% to boost revenue)
        core_count = max(1, int(n * 0.9))
        for entry in scored_stores[:core_count]:
            if entry['store'].restaurant_id not in used_ids:
                selected.append(entry['store'])
                used_ids.add(entry['store'].restaurant_id)
                if len(selected) >= core_count:
                    break
        
        # Fill remaining with fairness-boosted stores
        fairness_sorted = sorted(
            [s for s in scored_stores if s['store'].restaurant_id not in used_ids],
            key=lambda x: (x['fairness'], x['score']),
            reverse=True
        )
        
        for entry in fairness_sorted:
            if len(selected) >= n:
                break
            selected.append(entry['store'])
            used_ids.add(entry['store'].restaurant_id)
